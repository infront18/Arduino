...
char server[] = "192.168.23.50"; //접속할 서버 주소 APMSETUP이 실행중인 컴퓨터의 ip

 

IPAddress ip(192,168,23,21);     //아두이노에 부여하는 ip (고정ip부여함)
...
void loop() {
  ...
  if(humidity!=0 && temperature!=0) //온습도가 0이면 MYSQL에 값 보내지 마
  {
    if (client.connect(server, 80)) {   // Http 요청 헤더
      client.print( "GET /insert_test.php?"); //읽을 PHP 파일
      ...
      client.println( "Host: 192.168.23.50" ); //요청을 보낼 서버의 주소
